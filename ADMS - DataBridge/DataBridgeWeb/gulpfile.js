...
gulp.task('bundle-vendors-js', function () {
    return gulp.src(['js/vendors/jquery.js',        
        'js/vendors/angularJS/angular-1.5.7.js',
        'js/vendors/angularJS/angular-animate-1.5.7.js',
        'js/vendors/angularJS/angular-route.js',        
        'js/vendors/ui-bootstrap.js',
        'js/vendors/bootstrap-datepicker.min.js',
        'js/vendors/angularJS/**/*.js',
        'js/vendors/plugins/**/*.js',
        'js/vendors/**/*.js'])
...
gulp.task('bundle-databridge-js', function () {
    return gulp.src(['js/dataBridge/others/**/*.js',
            'js/dataBridge/others/app.js',
            'js/dataBridge/modules/*(.module)*.js',
            'js/dataBridge/services/*(.service)*.js',
            'js/dataBridge/factory/*(.factory)*.js',
            'js/dataBridge/directives/*(.directive)*.js',
            'js/dataBridge/controllers/controllers.js',
            'js/dataBridge/controllers/*(.controller)*.js',
            '!js/dataBridge/**/*.min.map',
            '!js/dataBridge/**/*.min.js.map'])
        .pipe(concat('dataBridgeApp.js'))
        .pipe(gulp.dest('js/'))
        .pipe(uglify())
        .pipe(rename({
            suffix: ".min",
...
gulp.task('hinting-js', function (done) {
    ...
    gulp.src(['js/dataBridge/**/*.js',
                '!js/dataBridge/**/*.min.js',
                '!js/dataBridge/**/*.min.map',
                '!js/dataBridge/**/*.min.js.map'])
      .pipe(jshint({ // Look at other options to disable / enable hinting rules here > http://jshint.com/docs/options/
      ...
      .on('end', function () {
          if (failed) {
              return done(new gutil.PluginError(
                  'jshint',
                  '############## Failed JSHint, fix errors and warnings ##############',
                  { showStack: false }
              ));
          }
